...
#define C_S(x,y) C_S[x*N+y]
#define C_P(x,y) C_P[x*N+y]
#define a(x,y) a[x*K+y]
#define b(x,y) b[x*n+y]
...
float  *a,*b,*c,*buffer;
int s;
float *A,*B,*C_S,*C_P;            /* A[M,K],B[P,N].正确的情况下K应该等于P,否则无法进行矩阵相乘 */
int M,N,K ;
int m,n;
int myid;
int p;                     /* 保存工作站集群中处理器数目，也即通信子大小 */
FILE *fp;            /* 用于读取输入文件内容和将计算结果输出到结果文件的临时文件指针 */
MPI_Status status;
double starttime,S_endtime,P_endtime;
...
void readData()
{
    int i,j,k;
   fp=fopen("data.txt","r");
  fscanf(fp,"%d%d%d",&M,&N,&K);              /* 读取矩阵的行，列数M,K */
   fclose(fp);
    A=(float *)malloc(floatsize*M*K);             /* 为矩阵A分配空间 */
    B=(float *)malloc(floatsize*K*N);             /* 为矩阵B分配空间 */
    C_S=(float *)malloc(floatsize*M*N);             /* 串行结果矩阵C_S[M,N]分配空间 */
    C_P=(float *)malloc(floatsize*M*N);            /* 并行结果矩阵C_S[M,N]分配空间 */
...
*/
  // printf("CP0.0\n");
    starttime = MPI_Wtime();
    for(i=0;i<M;i++)  //A's row
        for(j=0;j<N;j++) // B'column
            for ( k = 0;k <K ;k++)
               C_S(i,j)+=A(i,k)*B(k,j);
   // printf("CP1\n");
    S_endtime=MPI_Wtime();
...
{
    int i,j;
    printf("\n\nOutput of Matrix C for Serial = AB\n");
/*    for(i=0;i<M;i++)                              /* 输出C矩阵的结果数据 
  {
        for(j=0;j<N;j++) printf("%f\t",C_S(i,j));
        printf("\n");
    }
    printf("\n\n\nOutput of Matrix C for paralle = AB\n");
    for(i=0;i<M;i++)                              /* 输出C矩阵的结果数据 */
/*    {
        for(j=0;j<N;j++) printf("%f\t",C_P(i,j));
        printf("\n");
    }
*/
    P_endtime=MPI_Wtime();
    printf("\n");
   double  Serial_compute_time=P_endtime-starttime;
   double  Parallel_compute_time=P_endtime-S_endtime;
    printf("Whole running time    = %f seconds\n",P_endtime-starttime);
    printf("Serial coputer time  = %f seconds\n",Serial_compute_time);
    printf("Parallel compute time = %f seconds\n",Parallel_compute_time);
    printf("speed up for %d*%d is %f\n:",M,N,Serial_compute_time/Parallel_compute_time);
printf("1:%f 2:%f\n",C_S(0,0),C_P(0,0));
}
