...
int main (int argc, char *argv[])
{
	...
	//Construct csma topo
	CsmaHelper csma,csmator1,csmator2;
	csma.SetChannelAttribute ("DataRate", StringValue ("1Mbps"));
	csma.SetChannelAttribute ("Delay", TimeValue (NanoSeconds (500)));

        csmator1.SetChannelAttribute ("DataRate", StringValue ("1Mbps"));
	csmator1.SetChannelAttribute ("Delay", TimeValue (NanoSeconds (500)));

        csmator2.SetChannelAttribute ("DataRate", StringValue ("1Mbps"));
	csmator2.SetChannelAttribute ("Delay", TimeValue (NanoSeconds (500)));
	
	//middle topo
	NetDeviceContainer  deviceAggToR1,deviceAggToR2;
	deviceAggToR1=csmator1.Install (aggrtoR1);
	deviceAggToR2=csmator2.Install (aggrtoR2);
   ...
   csma.EnablePcap ("Pattern2 n8 to n1", deviceCsmaNodes4.Get (2), true);
      ...
      ptp1.EnablePcap ("pattern2 c1",devicePtp1.Get (0), true);
      ptp1.EnablePcap ("pattern2 a1",devicePtp1.Get (1), true);
      ptp2.EnablePcap ("pattern2 a2",devicePtp2.Get (1), true);  

      csma.EnablePcap ("pattern2 t1",deviceAggToR1.Get (1), true);
      csma.EnablePcap ("pattern2 t3",deviceAggToR2.Get (1), true);
      csma.EnablePcap ("pattern2 t2",deviceAggToR1.Get (2), true);
      csma.EnablePcap ("pattern2 t4",deviceAggToR2.Get (2), true);

      //ptp1.EnablePcapAll("Pattern2 ptp1");
      //ptp2.EnablePcapAll("Pattern2 ptp2");

//     csmator1.EnablePcapAll("Pattern2 top1");
  //    csmator2.EnablePcapAll("Pattern2 top2");
