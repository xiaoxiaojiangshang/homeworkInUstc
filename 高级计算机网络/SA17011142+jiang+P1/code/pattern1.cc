...

{
    ...
    char buf[50];
    CommandLine cmd;
    cmd.AddValue("DataRate1", "data rate of csma network at level 1", DataRate1);
    cmd.AddValue("DataRate2", "data rate of csma network at level 2", DataRate2);
    cmd.AddValue("verbose", "Tell sink and onoff applications to log if true", verbose);
    ...
    ptp1.SetDeviceAttribute ("DataRate", StringValue (buf));
    ...
    csma.SetChannelAttribute ("DataRate", StringValue (buf));
...
/////n5--n1 sinkapp
   PacketSinkHelper packetSinkHelper5("ns3::TcpSocketFactory", InetSocketAddress(interfacesCsma1.GetAddress(1),8080));
    ApplicationContainer sinkApp5 = packetSinkHelper5.Install(csmaNodes1.Get(1));

    sinkApp5.Start(Seconds(0.0));
    sinkApp5.Stop(Seconds(20.0));
    ...
    ApplicationContainer clientApp5 = client5.Install (csmaNodes1.Get(1));
    clientApp5.Start(Seconds (1.0 ));
    clientApp5.Stop (Seconds (21.0));

    csma.EnablePcap ("Pattern1 n5 to n1 ", deviceCsmaNodes1.Get (1), true);
/////
...
/////n2 -- n6 client
  PacketSinkHelper packetSinkHelper6("ns3::TcpSocketFactory",InetSocketAddress(interfacesCsma3.GetAddress(2),8080));
    ApplicationContainer sinkApp6 = packetSinkHelper6.Install(csmaNodes3.Get(2));
    sinkApp2.Start(Seconds(0.0));
    sinkApp2.Stop(Seconds(20.0));

    OnOffHelper client6("ns3::TcpSocketFactory", InetSocketAddress(interfacesCsma3.GetAddress(2), 8080));
    ...
    ApplicationContainer clientApp6 = client6.Install (csmaNodes1.Get(2));
    clientApp6.Start(Seconds (1.0 ));
    clientApp6.Stop (Seconds (21.0));

    csma.EnablePcap ("Pattern1 n2 to n6 ", deviceCsmaNodes3.Get (2), true);
/////
...
/////n7 --n3
  PacketSinkHelper packetSinkHelper7("ns3::TcpSocketFactory",InetSocketAddress(interfacesCsma2.GetAddress(1),8080));
    ApplicationContainer sinkApp7 = packetSinkHelper7.Install(csmaNodes2.Get(1));
    sinkApp7.Start(Seconds(0.0));
    sinkApp7.Stop(Seconds(20.0));

    OnOffHelper client7("ns3::TcpSocketFactory", InetSocketAddress(interfacesCsma2.GetAddress(1), 8080));
    ...
    ApplicationContainer clientApp7 = client7.Install (csmaNodes4.Get(1));
    clientApp7.Start(Seconds (1.0 ));
    clientApp7.Stop (Seconds (21.0));

    csma.EnablePcap ("Pattern1 n7 to n3 ", deviceCsmaNodes2.Get (1), true);
/////
...
////
  PacketSinkHelper packetSinkHelper8("ns3::TcpSocketFactory",InetSocketAddress(interfacesCsma4.GetAddress(2),8080));
    ApplicationContainer sinkApp8 = packetSinkHelper8.Install(csmaNodes4.Get(2));
    sinkApp8.Start(Seconds(0.0));
    sinkApp8.Stop(Seconds(20.0));

    OnOffHelper client8("ns3::TcpSocketFactory", InetSocketAddress(interfacesCsma4.GetAddress(2), 8080));
    ...
    client8.SetAttribute ("DataRate", DataRateValue (DataRate ("1.0Mbps")));
    client8.SetAttribute ("PacketSize", UintegerValue (2000));

    ApplicationContainer clientApp8 = client8.Install (csmaNodes2.Get(2));
    clientApp8.Start(Seconds (1.0 ));
    clientApp8.Stop (Seconds (21.0));

    csma.EnablePcap ("Pattern1 n4 to n8 ", deviceCsmaNodes4.Get (2), true);
////
////
    ...
    csma.EnablePcap ("pattern1 t2",deviceAggToR1.Get (2), true);
    csma.EnablePcap ("pattern1 t4",deviceAggToR2.Get (2), true);
